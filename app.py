...
# ---------- FEATURE (F): Percentile chart (Tableau-style) ----------
# Make a brand-new figure; do NOT reuse the one-pager fig
W_F, H_F = 1600, 1100
figF = plt.figure(figsize=(W_F/100, H_F/100), dpi=100)
figF.patch.set_facecolor("#0a0f1c")

# Try to use Yu Gothic; fall back silently if not present on the server
title_font = dict(fontsize=22, fontweight="bold", family="Yu Gothic UI Semibold")
label_font = dict(fontsize=11, fontweight="bold", family="Yu Gothic UI Semibold")

# Helper to get percentile + value safely
def _pct(metric):
    col = f"{metric} Percentile"
    if col in player_row.columns and pd.notna(player_row[col].iloc[0]):
        return float(player_row[col].iloc[0])
...
# ---- Define your metric groups (match your dataset column names) ----
ATTACKING_METRICS = [
    "Crosses per 90", "Accurate crosses, %", "Non-penalty goals per 90", "xG per 90",
    "Goal conversion, %", "xA per 90", "Offensive duels per 90", "Offensive duels won, %",
    "Shots per 90", "Shots on target, %", "Touches in box per 90",
]
DEFENSIVE_METRICS = [
    "Aerial duels per 90", "Aerial duels won, %", "Defensive duels per 90",
    "Defensive duels won, %", "PAdj Interceptions", "Successful defensive actions per 90",
]
POSSESSION_METRICS = [
...
def _panel(fig, title, metrics, y_top):
    n = len(metrics)
    # Axes for bars
    ax = fig.add_axes([left, y_top - panel_h, right - left, panel_h])
    ax.set_facecolor("#0a0f1c")
    ax.set_xlim(0, 100)
    ax.set_ylim(-0.5, n - 0.5)

    # Gridlines (white @ 10% alpha)
    ax.set_axisbelow(True)
    ax.grid(axis="x", linestyle="-", linewidth=1.0, color="#FFFFFF", alpha=0.10)
    ax.set_yticks([])

    # 50th percentile line (white 100%)
    ax.axvline(50, color="#FFFFFF", linewidth=2.0)

    # Bars + labels
    y_idx = np.arange(n)[::-1]  # top-to-bottom
    # track bg
    for yi in y_idx:
        ax.add_patch(mpatches.Rectangle((0, yi-0.4), 100, 0.8, facecolor="#11161C", edgecolor="none"))

    # left gutter for metric names
    gutter = 0.27  # fraction of axis width reserved visually for names
    # draw percent bars (use your same color mapping as elsewhere)
    def _bar_color(p):
        if np.isnan(p): return (0.6, 0.63, 0.66)
        if p <= 50:
            t = p/50.0;  c1, c2 = np.array([239,68,68]),  np.array([234,179,8])
        else:
            t = (p-50)/50.0; c1, c2 = np.array([234,179,8]), np.array([34,197,94])
        return tuple(((c1 + (c2-c1)*t)/255.0).astype(float))

    for yi, m in zip(y_idx, metrics):
        p = float(np.nan_to_num(_pct(m), nan=0.0))
        # bar (start at gutter*100 so names are clear of the bar)
        ax.add_patch(mpatches.Rectangle((gutter*100, yi-0.18), (100 - gutter*100)*(p/100.0), 0.36,
                                        facecolor=_bar_color(p), edgecolor="none"))
        # metric names (left), values (on bar start)
        figF.text(left + 0.002, (y_top - panel_h) + ((yi + 0.5)/n)*panel_h,
                  m.replace(" per 90","").replace(", %"," %"),
                  color="#FFFFFF", ha="left", va="center", **label_font)

    # Title aligned with left
    figF.text(left, y_top + 0.01, title, color="#FFFFFF", ha="left", va="bottom", **title_font)

# Draw panels
y_here = top_y
for title, mets in groups:
    _panel(figF, title, mets, y_here)
    y_here -= (panel_h + v_gap)

# Caption once at the bottom, centered
figF.text(0.5, 0.035, "Percentile Rank", color="#FFFFFF", ha="center", va="center",
          fontsize=11, fontweight="bold", family="Yu Gothic UI Semibold")

# Render & download (unique key!)
st.pyplot(figF, use_container_width=True)
bufF = BytesIO()
figF.savefig(bufF, format="png", dpi=170, bbox_inches="tight", facecolor=figF.get_facecolor())
st.download_button("⬇️ Download percentile chart (PNG)", data=bufF.getvalue(),
                   file_name=f"{str(player_name).replace(' ','_')}_percentiles.png",
                   mime="image/png", key="dl_percentile_png")
plt.close(figF)  # important: do not keep this figure alive
