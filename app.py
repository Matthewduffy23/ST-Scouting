# advanced_scouting_app.py — Advanced Scouting Tool
...
# Four output tables in tabs + single-player role profile
...
# Roles + metric weights (from your script)
ROLES = {
    'Target Man CF': {
        'metrics': {
...
with st.sidebar:
    ...
    # Position scope (defaults to CF, but you can broaden)
...
# League strength mapping + range filter
...
# League-wise percentiles for each feature
...
def compute_weighted_role_score(df_in: pd.DataFrame, role_name: str, metrics: dict, beta: float, league_weighting: bool) -> pd.Series:
    # Weighted average of selected feature percentiles
    ...
    if league_weighting:
        league_scaled = (df_in["League Strength"].fillna(50) / 100.0) * 100.0  # to 0..100
...
# Tabs: 1) Overall 2) U23 3) Expiring 4) Value Band
...
if player_row.empty:
    ...
    st.caption(f"**{player_name}** — {meta['Team']} • {meta['League']} • Age {int(meta['Age'])} • "
