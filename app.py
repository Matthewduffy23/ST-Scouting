...
if player_row.empty:
    ...
    def pct_to_rgb(v):
        # green → amber → red ramp (same as one-pager feel, mapped to 0–100)
        v = float(v)
        if v <= 50:
    ...
    probe.remove()
    gutter = min(0.26, lab_w + 0.02)

    # x-domain & ticks
    x_min, x_max = 0, 100
    ticks = np.arange(0, 101, 10)

    # --- panel drawer (with requested changes) ---
    def draw_panel(panel_top, title, tuples, *, show_xticks=False):
        ...
        # --- Tracks, bars, and value labels (inside-left, white) ---
        for i, (lab, pct, val_str) in enumerate(tuples[::-1]):  # reversed so first is top
            y = i
            # track
            ...
            # value INSIDE the bar near left edge, in white
    ...
    # lay down panels from top to bottom; only the last shows tick labels
    y_top = 1 - top_margin
    for idx, (title, data) in enumerate(sections):
        is_last = (idx == len(sections) - 1)
        y_top = draw_panel(y_top, title, data, show_xticks=is_last)
